...
<template>
  ...
  <Teleport to="body">
    <div class="fixed inset-x-0 top-12 bottom-0 z-90 transition-opacity duration-200 ease-out" :class="isExamSidebarOpen
      ? 'opacity-100 pointer-events-auto'
      : 'opacity-0 pointer-events-none'
      " aria-hidden="true" @click="isExamSidebarOpen = false" />
    ...
    <aside
      ...
      :class="isExamSidebarOpen ? 'translate-x-0' : '-translate-x-full'" aria-label="Välj tenta">
      <div class="flex h-full min-h-0 flex-col">
        ...
        <div ref="examListRef"
          ...
          :class="isExamSidebarOpen && sidebarContentReady
            ? 'opacity-100'
            : 'opacity-0'
            ">
          <div v-if="sortedExams.length === 0" class="px-2 py-4 text-sm text-muted-foreground">
            Inga tentor hittades.
          </div>

          <div v-else class="space-y-1">
            <button v-for="e in sortedExams" :key="e.id" :data-current="e.id.toString() === examId"
              class="group w-full cursor-pointer rounded-md border px-3 py-3 text-left transition-colors" :class="e.id.toString() === examId
              ...
              <div class="flex items-start justify-between gap-3">
                <div class="min-w-0 flex-1">
                  <div class="flex items-center gap-2">
                    ...
                    <span v-if="e.id.toString() === examId"
