...
<template>
  <div class="grid gap-2" :style="{ gridTemplateColumns: `repeat(${options.length}, minmax(0, 1fr))` }">
    ...
    <Button
      ...
      :key="option.value"
      variant="outline"
      class="h-auto flex-col items-start justify-start gap-0.5 rounded-2xl px-4 py-3 text-left whitespace-normal hover:border-primary hover:bg-primary/5"
      :class="model === option.value ? 'border-primary bg-primary/5' : 'bg-card'"
