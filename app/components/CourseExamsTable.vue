...
<template>
  <div class="flex flex-col gap-4 w-full">
    <div v-if="prefixes.length > 1" class="flex flex-wrap gap-2 w-full">
      <UButton v-for="p in prefixes" :key="p" :color="activeFilters.has(p) ? 'primary' : 'neutral'"
        :variant="activeFilters.has(p) ? 'solid' : 'outline'" size="sm" @click="toggleFilter(p)">
        {{ p }}
      </UButton>
    </div>
    ...
    <div class="w-full overflow-x-auto rounded-lg border border-default">
      <div class="w-max min-w-full sm:w-full rounded-lg overflow-hidden">
        <div :class="gridCols" class="py-3 border-b border-default/60 bg-muted/30">
          <div class="text-xs text-muted">Tentamen</div>
          <div class="text-xs text-muted">Typ</div>
          <div class="text-xs text-muted text-center">Facit</div>
          <div class="text-xs text-muted text-right">Godkänd</div>
        </div>
        ...
        <div v-for="exam in filteredExams" :key="exam.id" :class="gridCols"
          ...
          <div class="sm:min-w-0">
            <div
              class="text-sm font-semibold text-highlighted group-hover:text-highlighted/80 transition-colors whitespace-nowrap sm:truncate">
              {{ exam.exam_name }}
            </div>
            <div class="text-xs text-muted/70 mt-0.5 whitespace-nowrap">
              {{ exam.exam_date }}
            </div>
          </div>
          ...
          <div>
            <span v-if="getExamPrefix(exam)"
              class="text-2xs px-2 py-0.5 rounded-md border border-default bg-muted/40 text-muted font-mono">
              {{ getExamPrefix(exam) }}
            </span>
          </div>

          <div class="flex justify-center">
            <UIcon name="i-lucide-check" v-if="exam.has_solution" class="w-4 h-4 text-success" />
