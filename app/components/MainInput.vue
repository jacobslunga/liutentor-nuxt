...
<template>
  <div class="relative w-full">
    <div class="w-full relative flex flex-row items-center justify-center px-4">
      <LucideSearch size="28" class="text-muted-foreground" />
      ...
      <DropdownMenu>
        ...
        <DropdownMenuContent align="end" :side-offset="8" class="w-36 rounded-2xl">
          <DropdownMenuItem class="cursor-pointer rounded-xl" @click="selectSearchMethod('tentor')">
            <LucideCheck class="h-4 w-4" :class="searchMethod === 'tentor' ? 'opacity-100' : 'opacity-0'" />
            Tentor
          </DropdownMenuItem>
          <DropdownMenuItem class="cursor-pointer rounded-xl" @click="selectSearchMethod('stats')">
