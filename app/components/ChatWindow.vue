...
function handleScroll() {
  ...
  if (selectionPopover.value.visible) {
    const delta = Math.abs(
      el.scrollTop - selectionPopoverScrollAnchor.value,
    );
    if (delta > 80) {
      selectionPopover.value.visible = false;
    }
        ...
        <div ref="messagesContainer" class="h-full w-full overflow-y-auto overflow-x-hidden px-4 pt-20 custom-scrollbar"
          ...
          <div v-if="messages.length === 0"
            ...
            <p class="text-muted-foreground text-sm max-w-70 sm:max-w-md mb-8 leading-relaxed">
              Ställ frågor om tentan, be om ledtrådar eller få
              hjälp att förstå lösningarna.
            </p>
            ...
            <div v-for="(msg, i) in messages" :key="i" :class="msg.role === 'user' ? 'flex justify-end' : ''
              ...
              <div v-else class="w-full px-1 py-2" data-role="assistant">
                ...
                <div class="prose font-normal dark:prose-invert prose-p:font-normal prose-headings:font-medium prose-strong:font-medium prose-hr:border-foreground/10
                  prose-table:w-full prose-table:border-separate prose-table:border-spacing-0 prose-table:border prose-table:border-foreground/10 prose-table:rounded-2xl prose-table:overflow-hidden
                  prose-thead:bg-muted/50
                  prose-th:text-left prose-th:font-medium prose-th:px-4 prose-th:py-2.5 prose-th:border-b prose-th:border-foreground/10
                  prose-td:px-4 prose-td:py-2.5 prose-td:border-b prose-td:border-foreground/10 prose-td:align-top
                  marker:text-foreground marker:font-medium" v-html="renderedAssistantHtml[i]" />
              </div>
            </div>
