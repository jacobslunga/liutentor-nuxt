...
function handleMessageMouseUp() {
  setTimeout(() => {
    ...
    selectionPopoverScrollAnchor.value = messagesContainer.value?.scrollTop ?? 0;
          ...
          <div v-else class="space-y-6 max-w-2xl mx-auto w-full">
            <div v-for="(msg, i) in messages" :key="i" :class="msg.role === 'user' ? 'flex justify-end' : ''
              ...
              <div v-else class="w-full px-1 py-2" data-role="assistant">
                ...
                <div
                  class="prose dark:prose-invert prose-hr:border-foreground/10 prose-tr:border-foreground/10 prose-thead:border-foreground/10 marker:text-foreground marker:font-medium"
